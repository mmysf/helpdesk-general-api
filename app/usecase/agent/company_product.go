...
func (u *agentUsecase) GetCompanyProductList(ctx context.Context, claim domain.JWTClaimAgent, options map[string]interface{}) response.Base {
...
func (u *agentUsecase) GetCompanyProductDetail(ctx context.Context, claim domain.JWTClaimAgent, options map[string]interface{}) response.Base {
	ctx, cancel := context.WithTimeout(ctx, u.contextTimeout)
	defer cancel()
...
func (u *agentUsecase) CreateCompanyProduct(ctx context.Context, claim domain.JWTClaimAgent, options map[string]interface{}) response.Base {
	ctx, cancel := context.WithTimeout(ctx, u.contextTimeout)
	defer cancel()

	// check agent company type
	if claim.Company.Type == "B2C" {
		return response.Error(http.StatusBadRequest, "company type is not B2B")
	}
...
func (u *agentUsecase) UpdateCompanyProduct(ctx context.Context, claim domain.JWTClaimAgent, options map[string]interface{}) response.Base {
	ctx, cancel := context.WithTimeout(ctx, u.contextTimeout)
	defer cancel()
	...
	// update product
	newProduct := model.CompanyProduct{
		...
		Name:           payload.Name,
		Code:           strings.ToUpper(payload.Code),
		TicketTotal:    companyProduct.TicketTotal,
		LastActivityAt: companyProduct.LastActivityAt,
		CreatedAt:      time.Now(),
		UpdatedAt:      time.Now(),
	}

	err = u.mongodbRepo.UpdateCompanyProduct(ctx, &newProduct)
...
func (u *agentUsecase) DeleteCompanyProduct(ctx context.Context, claim domain.JWTClaimAgent, options map[string]interface{}) response.Base {
	ctx, cancel := context.WithTimeout(ctx, u.contextTimeout)
	defer cancel()
...
func (u *agentUsecase) UploadCompanyProductLogo(ctx context.Context, claim domain.JWTClaimAgent, payload domain.UploadAttachment, request *http.Request) response.Base {
	ctx, cancel := context.WithTimeout(ctx, u.contextTimeout)
	defer cancel()
