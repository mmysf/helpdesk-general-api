...
func (cj *cronjob) AutoCloseResolvedTickets() {
	cj.cron.AddFunc("*/1 * * * *", func() {
		...
		threeDaysAgo := time.Now().Add(-7 * 24 * time.Hour)

		// Filter tiket yang statusnya 'resolved' dan 'updated_at' lebih dari 3 hari
		fetchOptions := map[string]interface{}{
			"status":    []string{"resolved"},
			"updatedAt": map[string]interface{}{"$lt": threeDaysAgo},
		}
		...
		for cur.Next(cj.ctx) {
			...
			if ticket.UpdatedAt.Before(threeDaysAgo) && ticket.Status == model.Resolve {
				// Find company
